...
public class CommonElements3 {
	...
	public static void main(String[] args) {
		...
		CommonElements3 ts = new CommonElements3();
	...
	public ArrayList<Integer> solve(ArrayList<Integer> A, ArrayList<Integer> B) {
		 ...
		 */
		  ...
		  Map<Integer,Long> innerMapA = A.stream().collect(Collectors.groupingBy(Function.identity(),Collectors.counting())); 
		  ArrayList<Integer> result = new ArrayList<>();

		  
		  for(Integer valB: B) {
			  if(innerMapA.containsKey(valB)) {
				  Long count = innerMapA.get(valB);
				  if(count>0) {
					  result.add(valB);
				  } 
